...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
    <div class="mb-8">
        <p class="text-hearth-500 text-sm font-semibold uppercase tracking-wider mb-2">{{ __('Your Collection') }}</p>
        <h1 class="font-serif text-3xl md:text-4xl font-bold text-hearth-800">{{ __('Favorite Cafes') }}</h1>
    </div>
    ...
    @else
        <div class="text-center py-20">
            ...
            <h3 class="font-serif text-xl font-semibold text-hearth-800 mb-2">{{ __('No favorites yet') }}</h3>
            <p class="text-hearth-400 mb-6">{{ __('Start exploring and save cafes you love.') }}</p>
            <a href="{{ route('discover') }}" class="btn-primary">{{ __('Discover Cafes') }}</a>
        </div>
    @endif
</section>
@endsection
