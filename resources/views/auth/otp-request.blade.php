...
<head>
    ...
    <title>{{ __('Reset Password') }} - CoffeSpot</title>
    ...
    <div class="min-h-screen flex">
        <div class="w-full lg:w-1/2 flex items-center justify-center p-8">
            <div class="w-full max-w-md">
                ...
                <div class="mb-8">
                    <h1 class="font-serif text-3xl font-bold text-hearth-800 mb-2">{{ __('Reset Password') }}</h1>
                ...
                <form method="POST" action="{{ route('password.otp.send') }}" class="space-y-5">
                    ...
                    <button type="submit" class="btn-primary w-full text-center">{{ __('Send OTP') }}</button>
                </form>

                <p class="text-center text-sm text-hearth-400 mt-6">
                    <a href="{{ route('login') }}" class="font-semibold text-hearth-600 hover:text-hearth-800">{{ __('Back to Sign In') }}</a>
                </p>
            </div>
        </div>
    </div>
